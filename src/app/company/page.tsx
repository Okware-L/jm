...
export default function CompanyPage() {
  const { state, profile } = useRequireRole(["company_admin"]);

  if (state === "loading" || !profile) {
    return <LoadingScreen label="Opening company workspace" />;
  }

  return <CompanyWorkspace profile={profile} />;
}

function CompanyWorkspace({ profile }: { profile: UserProfile }) {
  const { workspace, loading, reload } = useWorkspaceData("company_admin", profile);
  const {
    busyAction,
    notice,
    dialog,
  ...
  if (loading) {
    return <LoadingScreen label="Loading live company records" />;
  }
